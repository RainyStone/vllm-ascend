---
name: vllm-debug-evidence-based
description: 分析 vLLM 服务问题的工作方式：基于代码与日志取证，不瞎猜；日志不足时先与用户确认加日志、由用户跑实验提供数据
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 1a1608ce-a48f-4def-b9c9-180e664c2d7f
---

分析 vLLM 服务问题、解释代码行为时，必须基于**代码 + 日志**取证，不得瞎猜；**文档（docs/、设计文档）可作为参考与线索，但不能只看文档下结论，文档可能过时**，任何关键结论都要回到代码核实。

**Why:** 这是用户明确要求的工作方式，且经多次验证。集体通信死锁类问题因果链长、跨进程，凭推断给出的根因极易错（早期多次出现"按住葫芦浮起瓢"——fix 一边坏另一边），只有代码与日志的交叉证据才可靠。文档口径常落后于实现（例：PCP/DCP 的 Q/KV 切分行为，文档表述与 `common_cp.py`/`mla_cp.py`/`block_table.py` 实现存在出入），唯文档会得出错误结论。

**How to apply:**
- 推断根因或落改动前，先读相关代码、抽取日志数据交叉验证；明确区分"已证实"与"推断"。
- **回答"某机制如何实现"类问题时，去读代码（grep/read 实现文件），不要凭文档转述**；文档用作先验地图定位文件、理解设计意图，但下结论以代码为准。遇到文档与代码冲突，以代码为准并指出差异。
- 若现有日志信息不足、无法支撑结论，**不要继续猜**：及时与用户确认是否加辅助分析的日志打印，由用户跑实验提供新日志，再据此分析。
- 修复改动要能从日志验证（说明复跑后预期看到的具体日志现象）。

## 服务日志末尾的 "died with exit code None" 一般是手动 kill，非根因

分析 vLLM 服务日志时，日志**末尾**若出现 `Process VLLM_DP_Coordinator (PID: xxxx) died with exit code None`（或类似 "Process ... died with exit code None" + 后续 RuntimeError/ERR99999 栈），通常是**用户手动 kill** 进程造成的，不是真正的崩溃原因，分析时可忽略末尾这一段。

**Why:** 在 DyCP/DP 调试中，为停止卡死或测试结束，常手动 kill 协调器/引擎进程，kill 会被监控捕获并打印这类 "died with exit code None" 异常栈，淹没真正的故障点。

**How to apply:** 排查时先定位日志中**末尾**的 "died with exit code None"，确认它是手 kill 产生后忽略；真正的异常/卡死点在它**之前**。不要把这段当作根因。